...
<option className="text-dark bg-light" value="1">All Categories</option>
      ...
      */}
      ...
      {isAuthenticated ? ( <div onClick={Logout} className="d-flex flex-row m-3">
...
<div className="d-flex flex-row m-3" >
  ...
  <i className='bx bx-cart bx-lg text-light' ></i> <span className="position-absolute top-0 text-light start-100 translate-middle badge rounded-pill text-bg-info"> {cartItems.length}  <span className="visually-hidden">unread messages</span></span>
</div>
    </div>
    
    <div className="d-flex flex-column mb-3">
  <div className="text-light">Your Cart  </div>
  <div className="text-light">${getCartTotal()}</div>
...
<Container className="navbar-toggler border border-0 " fluid>
  ...
  <div className="px-1"><i className='bx bxs-group bx-lg text-dark' ></i></div>
  <div className="d-flex flex-column mb-3">
