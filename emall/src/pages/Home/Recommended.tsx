...
const recommended = useAppSelector(getRecommended)
  ...
  {recommended?.map((items:ITEM, id) =>{
     return (
       ...
       <Link onClick={() =>{dispatch(fetchAddRecentlyViewed(items)); dispatch(fetchAddRelated(items));   }}  to={`/product/${items.title}`} className="p-2 text-decoration-none text-reset"> <Image loading="lazy" src={items.image} width="150px" height="200px"  className="" /></Link> 
