...
function Payment() {
 ...
 const stripe = loadStripe(STRIPE)
  ...
  <div className=" fs-6 text-dark">Placed on On: { weekday[new Date(currentOrder.createdAt).getDay()]}, {currentOrder.createdAt.slice(0,10)}</div>
  ...
  <div className="p-1 fs-6 text-dark"><div className="text-dark">Delivery: {currentOrder.isDelivered ? "Delivery Completed" : "Pending"}</div> </div>
</div>
...
<a  href={`/orderdetails/${currentOrder._id}`} className=" text-decoration-none text-reset">SEE DETAILS</a>
        ...
        ) : (<h1 className='text-center py-5'>Payment page loading!</h1>)
        }
