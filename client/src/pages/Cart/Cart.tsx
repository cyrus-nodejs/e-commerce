...
setData({...data, [e.target.name] : e.target.value})
  ...
  const region =  currentState?.name
  const province = currentCity?.name
...
Arriving at {destination?.address} {destination?.province} between {date.toDateString()}  & {pickupDate.toDateString() }  when you order within next 4hrs 41mins
  ...
  {destination ?    (<Button variant="dark" className="shadow-none rounded-1" size="lg" type="submit" onClick={() => dispatch(fetchCreatePayment(orderdata={gift, shipping,    cartBills}))}>
      <Link to="/payment" className="text-decoration-none text-light">
       Make Payment
               ...
               {showAlert && (  <strong>Warning!  Please Create Address to proceed with payment</strong> )}
        <Button variant="dark" className="shadow-none rounded-1" size="lg"  onClick={handleShowAlert} type="submit" >
       Make Payment
      </Button>
      </div>
