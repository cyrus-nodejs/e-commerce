...
console.log(BASEURL)
      ...
      export const fetchAddReviewItem = createAsyncThunk(
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        'items/fetchReviewItem', async (data:any) => {
              const {productId, rating, comment} = data
            const response= await axios.post(`${BASEURL}/reviews`,{rating, productId, comment},{ withCredentials: true })
      ...
      export const fetchRecentlyViewed = createAsyncThunk(
        'items/fetchRecentlyViewed', async () => {
          ...
          export const fetchGetRelated= createAsyncThunk(
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            'items/fetchGetRelated', async (products:ITEM|null) => {
  ...
  export const fetchCategory = createAsyncThunk(
    'items/fetchCategory',  async () => {
      ...
      export const fetchCategoryItems = createAsyncThunk(
        'items/fetchCategoryItems',  async (id:string|undefined) => {
...
builder.addCase(fetchAddItem.pending, (state) => {
 ...
 .addCase(fetchAddReviewItem.pending, (state) => {
  state.status = 'pending'
  })
  .addCase(fetchAddReviewItem.fulfilled, (state, action) => {
  state.message = action.payload.message
  })
  .addCase(fetchAddReviewItem.rejected, (state, action) => {
  state.status = 'failed'
  state.error = action.error.message;
  })

.addCase(fetchUpdateItem.pending, (state) => {
  state.status = 'pending'
  })
...
state.userHasReviewed =action.payload.userHasReviewed
state.status = 'succeeded'
})
...
export const getCategoryItems =(state:RootState) => state.items.categoryItems
...
export const getUserHasReviewed = (state:RootState) => state.items.userHasReviewed
